...
import { resetLeaves, setallLeaves, settotalleaves } from "@/redux/leave/leaves";
...
export default function LeaveComponent() {
  ...
  const renderData = async (currpage: number, limit: number, search: string) => {
    dispatch(resetLeaves());
    const response = await trigger({ page: currpage, limit: limit, search: search });
    try {
      if (response.data) {
  ...
  return (
    <>
      ...
      <div className="flex justify-end">
        <CustomPagination handlePageChange={handlePageChange} page={currentpage} total={totalleaves} limit={tableDataLimit} />
      </div>
    </>
  );
}